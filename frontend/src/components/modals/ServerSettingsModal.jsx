...
export default function ServerSettingsModal({ server, onClose, onServerUpdated, onServerDeleted }) {
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-2 sm:p-4">
      ...
      {/* Modal Container */}
      <div className="relative bg-surface-base modal-width-5xl max-w-5xl h-[min(92dvh,42rem)] rounded-lg sm:rounded-xl shadow-2xl flex flex-col sm:flex-row overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        
        {/* Left Sidebar */}
        <div className="w-full sm:w-64 bg-surface-panel flex flex-col border-b sm:border-b-0 sm:border-r border-surface-border p-3 sm:p-4 shrink-0">
          <div className="text-xs font-bold text-text-muted uppercase tracking-wider mb-2 px-2">
            {server.name} Settings
          </div>
          
          <div className="flex sm:block gap-2 overflow-x-auto no-scrollbar responsive-safe-scroll">
          <button
            onClick={() => setActiveTab('overview')}
            className={`shrink-0 sm:w-full flex items-center space-x-3 px-3 py-2 rounded-md font-medium transition-colors mb-1 min-h-10 ${
              activeTab === 'overview' 
          ...
          <button
            ...
            className={`shrink-0 sm:w-full flex items-center space-x-3 px-3 py-2 rounded-md font-medium transition-colors min-h-10 ${
              activeTab === 'members' 
                ? 'bg-surface-active text-text-primary' 
                : 'text-text-muted hover:bg-surface-hover hover:text-text-primary'
            }`}
          ...
          <button
            ...
            className={`shrink-0 sm:w-full flex items-center space-x-3 px-3 py-2 rounded-md font-medium transition-colors mt-1 min-h-10 ${
          ...
          <div className="sm:mt-auto pt-3 sm:pt-4 border-t border-surface-border">
            <button
              onClick={handleDeleteServer}
              disabled={!isOwner}
              className={`w-full flex items-center justify-between px-3 py-2 rounded-md font-medium transition-colors min-h-10 ${
                isOwner 
        ...
        {/* Right Content Area */}
        <div className="flex-1 min-h-0 flex flex-col bg-surface-base relative">
          
          {/* Close Button */}
          <button
            onClick={onClose}
            className="absolute top-3 right-3 sm:top-4 sm:right-4 p-2 text-text-muted hover:text-text-primary hover:bg-surface-hover rounded-full transition-colors z-10 mobile-touch-target sm:min-w-0 sm:min-h-0"
          >
            <X className="w-5 h-5" />
          </button>

          <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 no-scrollbar responsive-safe-scroll">
            {error && (
            ...
            {activeTab === 'overview' && (
              <div className="max-w-xl">
                <h2 className="text-xl sm:text-2xl font-bold text-text-primary mb-6 pr-10">Server Overview</h2>
            ...
            {activeTab === 'roles' && (
              <div className="max-w-2xl">
                <h2 className="text-xl sm:text-2xl font-bold text-text-primary mb-6 pr-10">Server Roles</h2>
                ...
                <div className="space-y-2">
                  {roles.length === 0 ? (
                    ...
                    roles.map(role => (
                      <div key={role.id} className="flex items-center justify-between gap-3 p-3 bg-surface-active border border-surface-border rounded-md">
            ...
            {activeTab === 'members' && (
              <div className="max-w-2xl">
                <h2 className="text-xl sm:text-2xl font-bold text-text-primary mb-2 pr-10">Server Members</h2>
                ...
                <div className="space-y-2">
                  {members.map(member => {
                    ...
                    return (
                      <div key={member.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3 bg-surface-active border border-surface-border rounded-md hover:border-accent-primary/50 transition-colors">
                        <div className="flex items-center space-x-3 min-w-0">
                          <img src={avatarUrl} alt={member.user?.username} className="w-10 h-10 rounded-full bg-surface-panel object-cover" />
                          <div className="min-w-0">
                            <div className="font-bold text-text-primary flex items-center space-x-2 min-w-0">
            ...
            {activeTab === 'integrations' && (
              <div className="max-w-2xl">
                <div className="flex items-start justify-between gap-3 mb-6 pr-10 sm:pr-0">
                  <div>
                    <h2 className="text-xl sm:text-2xl font-bold text-text-primary mb-1">Webhooks & Integrations</h2>
                        ...
                        <div key={webhook.id} className="p-4 bg-surface-active border border-surface-border rounded-md flex flex-col space-y-4 hover:border-accent-primary/50 transition-colors">
                          <div className="flex items-center justify-between gap-3">
                            <div className="flex items-center space-x-3 min-w-0">
                              ...
                              <div className="min-w-0">
                                <div className="font-bold text-text-primary truncate">{webhook.name}</div>
                          ...
                          <div className="bg-surface-panel border border-surface-border rounded p-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                            <div className="font-mono text-xs text-text-muted truncate sm:mr-4 w-full">
