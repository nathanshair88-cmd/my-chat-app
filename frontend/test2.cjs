...
async function test() {
  try {
    ...
    // Create a server
    const serverRes = await axios.post('https://my-chat-backend-suva.onrender.com/api/servers', {
      name: 'Test Server',
      icon_url: ''
    ...
    // Get channels
    const serverDetails = await axios.get('https://my-chat-backend-suva.onrender.com/api/servers/' + server_id, {
      headers: { Authorization: 'Bearer ' + token }
    });
